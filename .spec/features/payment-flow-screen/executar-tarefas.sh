#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano payment-flow-screen` em 2026-08-11 11:50
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo payment-flow-screen
set -u
set -o pipefail

RUN_ID='poc-payment-payment-flow-screen-msolmcxk'
FEATURE='payment-flow-screen'
BASE_BRANCH='spec/payment-flow-screen'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/payment-flow-screen/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/poc-payment-payment-flow-screen-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/poc-payment-payment-flow-screen"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/payment-flow-screen/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/payment-flow-screen-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-005' 'Você executa UMA tarefa da feature "payment-flow-screen" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/payment-flow-screen/spec.md, .spec/features/payment-flow-screen/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-005 — "Criar a interface visual do fluxo"
  critérios/refs: AC-007 (A tela apresenta o fluxo e o resumo da operação), AC-008 (O meio de pagamento altera a etapa exibida), AC-012 (A tela funciona em viewport estreito)
  arquivos permitidos (e seus testes): web/payment-flow/index.html, web/payment-flow/styles.css
  mensagem de commit: "T-005 payment-flow-screen: Criar a interface visual do fluxo"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' low
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/payment-flow-screen-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/payment-flow-screen-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-006' 'Você executa UMA tarefa da feature "payment-flow-screen" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/payment-flow-screen/spec.md, .spec/features/payment-flow-screen/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-006 — "Implementar interações e validação local"
  critérios/refs: AC-009 (Campos obrigatórios são validados antes do avanço), AC-010 (O usuário consegue revisar e voltar sem perder os dados), AC-011 (A confirmação mostra o resultado da operação)
  arquivos permitidos (e seus testes): web/payment-flow/app.js, web/payment-flow/payment-flow-core.js
  mensagem de commit: "T-006 payment-flow-screen: Implementar interações e validação local"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' low &&
    rodar_tarefa 'faixa-2' 'T-007' 'Você executa UMA tarefa da feature "payment-flow-screen" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/payment-flow-screen/spec.md, .spec/features/payment-flow-screen/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-007 — "Criar testes unitários da lógica do fluxo"
  critérios/refs: AC-014 (A lógica do fluxo possui testes unitários)
  arquivos permitidos (e seus testes): web/payment-flow/tests/payment-flow.unit.test.mjs, web/payment-flow/payment-flow-core.js
  mensagem de commit: "T-007 payment-flow-screen: Criar testes unitários da lógica do fluxo"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' low
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/payment-flow-screen-faixa-2' "$WT" "$st" || return 1
  marcar_concluidas T-006 T-007
  return 0
}
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/payment-flow-screen-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-008' 'Você executa UMA tarefa da feature "payment-flow-screen" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/payment-flow-screen/spec.md, .spec/features/payment-flow-screen/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-008 — "Criar testes de integração do navegador"
  critérios/refs: AC-016 (O teste de integração cobre o fluxo completo)
  arquivos permitidos (e seus testes): web/payment-flow/tests/payment-flow.integration.spec.mjs, web/payment-flow/playwright.config.mjs, web/payment-flow/test-server.mjs, web/payment-flow/package.json, web/payment-flow/package-lock.json
  mensagem de commit: "T-008 payment-flow-screen: Criar testes de integração do navegador"
...
- Rode os testes localmente com `node .spec/run-tests.mjs` até passarem.
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' medium
  ) >> "$LOG_DIR/faixa-3.log" 2>&1
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/payment-flow-screen-faixa-3' "$WT" "$st" || return 1
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/payment-flow-screen-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-009' 'Você executa UMA tarefa da feature "payment-flow-screen" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/payment-flow-screen/spec.md, .spec/features/payment-flow-screen/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-009 — "Documentar e conectar as provas ao CI/CD"
  critérios/refs: AC-007 (A tela apresenta o fluxo e o resumo da operação), AC-008 (O meio de pagamento altera a etapa exibida), AC-009 (Campos obrigatórios são validados antes do avanço), AC-010 (O usuário consegue revisar e voltar sem perder os dados), AC-011 (A confirmação mostra o resultado da operação), AC-012 (A tela funciona em viewport estreito), AC-013 (A documentação explica como usar e testar a tela), AC-014 (A lógica do fluxo possui testes unitários), AC-015 (O CI/CD executa as camadas de teste da tela), AC-016 (O teste de integração cobre o fluxo completo)
  arquivos permitidos (e seus testes): docs/payment-flow.md, web/payment-flow/README.md, .spec/static-tests/payment-flow-screen.test.mjs, .spec/run-tests.mjs, onpspec.config.json, .github/workflows/ci.yml, .gitignore
  mensagem de commit: "T-009 payment-flow-screen: Documentar e conectar as provas ao CI/CD"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/payment-flow-screen-faixa-4' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/payment-flow-screen/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-2  onda 1  T-006, T-007"
  echo "  faixa-3  onda 1  T-008"
  echo "  faixa-4  onda 2  T-009"
  echo
