#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano aws-terraform-payment-poc` em 2026-08-09 12:01
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo aws-terraform-payment-poc
set -u
set -o pipefail
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/aws-terraform-payment-poc/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/poc-payment-aws-terraform-payment-poc-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/poc-payment-aws-terraform-payment-poc"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/aws-terraform-payment-poc/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/aws-terraform-payment-poc-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-001' 'Você executa UMA tarefa da feature "aws-terraform-payment-poc" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/aws-terraform-payment-poc/spec.md, .spec/features/aws-terraform-payment-poc/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-001 — "Reorganizar diretorios e escopo do onp-spec"
  critérios/refs: AC-001 (Terraform AWS fica em `infra/aws/terraform`)
  arquivos permitidos (e seus testes): onpspec.config.json, .gitignore
  mensagem de commit: "T-001 aws-terraform-payment-poc: Reorganizar diretorios e escopo do onp-spec"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' low
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/aws-terraform-payment-poc-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/aws-terraform-payment-poc-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-002' 'Você executa UMA tarefa da feature "aws-terraform-payment-poc" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/aws-terraform-payment-poc/spec.md, .spec/features/aws-terraform-payment-poc/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-002 — "Alinhar ECS, ECR e ALB aos tres servicos reais"
  critérios/refs: AC-002 (ECS e ECR usam somente os tres servicos da PoC), AC-005 (ALB roteia para endpoints reais)
  arquivos permitidos (e seus testes): infra/aws/terraform/versions.tf, infra/aws/terraform/data.tf, infra/aws/terraform/network.tf, infra/aws/terraform/locals.tf, infra/aws/terraform/variables.tf, infra/aws/terraform/ecs.tf, infra/aws/terraform/ecr.tf, infra/aws/terraform/alb_waf.tf, infra/aws/terraform/security.tf, infra/aws/terraform/outputs.tf
  mensagem de commit: "T-002 aws-terraform-payment-poc: Alinhar ECS, ECR e ALB aos tres servicos reais"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium &&
    rodar_tarefa 'faixa-2' 'T-003' 'Você executa UMA tarefa da feature "aws-terraform-payment-poc" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/aws-terraform-payment-poc/spec.md, .spec/features/aws-terraform-payment-poc/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-003 — "Trocar dependencias genericas por RDS, DocumentDB e Amazon MQ"
  critérios/refs: AC-003 (Terraform provisiona RDS, DocumentDB e RabbitMQ gerenciado), AC-004 (Variaveis de ambiente atendem Java, Kotlin e C#)
  arquivos permitidos (e seus testes): infra/aws/terraform/rds.tf, infra/aws/terraform/documentdb.tf, infra/aws/terraform/rabbitmq.tf, infra/aws/terraform/security.tf, infra/aws/terraform/iam.tf, infra/aws/terraform/outputs.tf, infra/aws/terraform/variables.tf, infra/aws/terraform/locals.tf
  mensagem de commit: "T-003 aws-terraform-payment-poc: Trocar dependencias genericas por RDS, DocumentDB e Amazon MQ"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/aws-terraform-payment-poc-faixa-2' "$WT" "$st" || return 1
  marcar_concluidas T-002 T-003
  return 0
}
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/aws-terraform-payment-poc-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-004' 'Você executa UMA tarefa da feature "aws-terraform-payment-poc" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/aws-terraform-payment-poc/spec.md, .spec/features/aws-terraform-payment-poc/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-004 — "Atualizar documentacao e provas estaticas da arquitetura"
  critérios/refs: AC-001 (Terraform AWS fica em `infra/aws/terraform`), AC-002 (ECS e ECR usam somente os tres servicos da PoC), AC-003 (Terraform provisiona RDS, DocumentDB e RabbitMQ gerenciado), AC-004 (Variaveis de ambiente atendem Java, Kotlin e C#), AC-005 (ALB roteia para endpoints reais), AC-006 (Documentacao AWS nao cita a arquitetura antiga de billing)
  arquivos permitidos (e seus testes): infra/aws/terraform/README.md, infra/aws/terraform/terraform.tfvars.example, infra/aws/terraform/observability.tf, docs/architecture.md, README.md, .github/workflows/ci.yml, .spec/run-tests.mjs, .spec/static-tests/aws-terraform-payment-poc.test.mjs
  mensagem de commit: "T-004 aws-terraform-payment-poc: Atualizar documentacao e provas estaticas da arquitetura"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' low
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/aws-terraform-payment-poc-faixa-3' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/aws-terraform-payment-poc/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-2  onda 1  T-002, T-003"
